...
In astrophysics, Gaussian processes have been used in various applications including in the study of photometric redshifts (Way et al. 2009), variability in stellar light curves (Angus et al. 2017), eclipses in cataclysmic variable stars (McAllister et al. 2016), transmission spectroscopy of extrasolar planets (P. Gibson et al. 2011), and radio velocity studies for planetary candidates (Rajpaul et al. 2015). It has also been used extensively in cosmic microwave background analysis (Bond \& Efstathiou 1987; Bond et al. 1999; Wandelt \& Hansen 2003), as well as for modelling instrumental systematics and calibration (e.g. Gibson et al. 2012; Dawson et al. 2014; Haywood et al. 2014; Barclay et al. 2015; Czekala et al. 2015; Evans et al. 2015; Haywood 2015; Rajpaul et al. 2015; Vanderburg et al. 2015; Aigrain, Parviainen \& Pope 2016; Rajpaul, Aigrain \& Roberts 2016; Littlefair et al. 2017). Here we consider the use of GPM for the reconstruction and interpretation of complex polarization data, specifically in the context of Faraday depth structure. 
...
10,000 Faraday thin sources were created with an RM taken from a uniform distribution between -100 and +100\,rad\,m$^{-2}$. Stokes~Q and U values were generated for 135 frequency channels spaced uniformly in wavelength from 0.86\,GHz to 1.70\,GHz. The intrinsic angle of polarisation for each source, $\chi_0$, was sampled randomly from a uniform distribution $[-\pi, +\pi)$. 
%Each polarisation spectrum was then used to find the Faraday spectrum for each source using RM Synthesis \cite{2005A&A...441.1217B}. 


At each frequency, noise was added to the Stokes~Q and U values using a normal distribution with a mean of zero and a variance of $1.32\times10^{-6}$\,Jy\,beam$^{-1}$, which was calculated using
%
\begin{equation}
\label{eq:S_rms}
S_{\rm rms} = \frac{2 k_{\rm B} T}{A\eta \sqrt{n_{\rm a}(n_{\rm a} - 1) \Delta \nu\tau_0}} \frac{w_{\rm rms}}{w_{\rm mean}},
\end{equation}
%
where $k_{\rm B}$ is Boltzmann's constant, the sensitivity is $\eta A/T = 220$\,m$^2$/K (\cite{2012AfrSk..16..101B}), $n_{\rm a}$ is the number of antennas, $\Delta \nu$ is the frequency channel width, and $\tau_0$ is the observation integration time. The integration time is assumed to be one hour, and the ratio ${w_{\rm rms}}/{w_{\rm mean}}$ is assumed to be unity, equivalent to natural weighting. This simulated data does not include any instrumental polarization leakage between the Stokes parameters.

Values for Stokes~I were taken informed by the extragalactic source counts models in \cite{2010A&ARv..18....1D}, which are shown in Figure \ref{fig:SourceCounts1}. These models cover flux density values up to 1\,Jy for Star Forming Galaxies (SFGs), and up to 148.4\,Jy for Active Galactic Nuclei (AGN). 
%
\begin{figure}
\centering
\includegraphics[width=0.45\textwidth]{./FIGURES/SourceCounts1.png}
\caption{\label{fig:SourceCounts1} Contributions from different source populations at 1.4\,GHz used for the simulations here. \cite{2010A&ARv..18....1D}. }
\end{figure}
...
\end{equation}
%
where $S_\nu$ is the flux density at each frequency, $\nu$, $S_{1.4\GHz}$ is the flux density at 1.4\,GHz, and $\alpha$ is the spectral index, which is taken to be the canonical value of $\alpha$ = 0.7 for synchrotron radiation. 
...
\end{equation}
%
\cite{Stil_2014}, where $\Pi_{0, {\rm med}}$ is the median percentage polarisation fraction and $S_{\rm 1.4\,GHz}$ is the 1.4\,GHz flux density in mJy. 
...
\end{table}
%
We evaluate the performance of the GP approach using the 16 models from Sun et~al. (2017). These models include examples of single Faraday thin objects, multiple (two) Faraday thin objects, and combinations of a Faraday thick and Faraday thin objects. 
...
The reduced $\chi^2$ for parametric models described in Sun et~al. (2017) is roughly equivalent to the standardized mean squared error (SMSE). For an optimised GP, the SMSE is defined as
%
\begin{equation}
...
In addition to the median reduced chi-squared statistic, we also define the {\it mean standardised log loss} (MSLL) as a measure of the prediction quality. As described by Rasmussen \& Williams (2004), this quantity is the difference between the negative log likelihood for the optimized GP, given by,
%
\begin{equation}
...
The median of the reduced chi-squared statistic recovered from our model, $\chi^2_{\rm r} = 0.99$, is in very good agreement with the expected optimum value of $1.00\pm0.02$ from Sun et~al. (2014), which is also consistent with the value of the $MSLL= -0.90$; however, the median $RM_{\rm wtd}=4.88$ statistic is not in good agreement with the expected optimum value of $1.3\pm0.5$. This is due to the GP interpreting some low local structure in the noise fluctuations of the input data as signal, which is reflected as structure in the Faraday depth spectrum below the detection threshold described in \S~\ref{sec:usage}. 
